...
export interface FileBrowserPanelHandle {
  /** Expand ancestors of a directory (or the whole tree for "") and scroll it into view. */
  revealDirectory: (relativePath: string) => void;
}
    ...
    useImperativeHandle(
      ...
      () => ({
        revealDirectory: (relativePath) => {
          ...
          let ancestorPath = "";
          let targetItem: ReturnType<typeof model.getItem> | undefined;
          for (const segment of segments) {
            ancestorPath = ancestorPath ? `${ancestorPath}/${segment}` : segment;
            const item = model.getItem(`${ancestorPath}/`) ?? model.getItem(ancestorPath);
            if (item && "expand" in item) item.expand();
            if (ancestorPath === relativePath) targetItem = item;
          }
          targetItem?.select();
          model.scrollToPath(`${relativePath}/`, { focus: true, offset: "center" });
        },
      }),
      [model],
    );
